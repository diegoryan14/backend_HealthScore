...
type RestOf<T extends IUsuario | NewUsuario> = Omit<T, 'dataRegistro' | 'dataNascimento'> & {
...
export class UsuarioService {
  ...
  protected convertDateFromClient<T extends IUsuario | NewUsuario | PartialUpdateUsuario>(usuario: T): RestOf<T> {
    return {
      ...usuario,
      dataRegistro: usuario.dataRegistro?.toJSON() ?? null,
      dataNascimento: usuario.dataNascimento?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restUsuario: RestUsuario): IUsuario {
    return {
      ...restUsuario,
      dataRegistro: restUsuario.dataRegistro ? dayjs(restUsuario.dataRegistro) : undefined,
      dataNascimento: restUsuario.dataNascimento ? dayjs(restUsuario.dataNascimento) : undefined,
    };
  }
